...
static void	load_images(t_game *game)
{
	...
	game->img_floor = mlx_xpm_file_to_image(game->mlx, "sprites/floor.xpm", &w,
			&h);
	game->img_player = mlx_xpm_file_to_image(game->mlx, "sprites/link.xpm", &w,
			&h);
	game->img_collectible = mlx_xpm_file_to_image(game->mlx, "sprites/ruby.xpm",
			&w, &h);
...
void	init_game(t_game *game)
{
	...
	game->win = mlx_new_window(game->mlx, game->map_width * TILE_SIZE,
			game->map_height * TILE_SIZE, "So Long with Link");
	if (!game->win)
	{
